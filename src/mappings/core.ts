...
export function handleDepositedUpdated(event: DepositedUpdated): void {
  ...
  pair.deposit0 = convertTokenToDecimal(event.params.deposited0, token0.decimals)
  pair.deposit1 = convertTokenToDecimal(event.params.deposited1, token1.decimals)
...
export function handleRedepositRatioUpdated(event: RedepositRatioUpdated): void {
  let pair = Pair.load(event.address.toHex())
  if (event.params.tokenIndex == 0) {
